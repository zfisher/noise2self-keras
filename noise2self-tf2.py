...
def train_model(clean_train, clean_test, noisy_test, num_batches=150, 
    ...
    for (batch, (batch_noisy, batch_clean)) in enumerate(noise_gen):
        ...
        with tf.GradientTape() as tape:
            ...
            mask_clean = mask * batch_clean
            mask_predictions = mask * batch_predictions
            loss_value = tf.reduce_mean(loss_fn(mask_clean, mask_predictions))
    ...
    mask_clean = mask * clean_test
    mask_predictions = mask * test_predictions
    test_loss_value = tf.reduce_mean(loss_fn(mask_clean, mask_predictions))
